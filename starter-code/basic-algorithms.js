...
const hacker2 = prompt ("Whats your name?")
  function nameLength (){
    if(hacker1.lenght > hacker2.length){
      console.log( `The ${hacker1} driver's has the longest name, it has ${hacker1.length} characters.`)
    }else if(hacker1.length < hacker2.length){
      console.log( `The ${hacker2} driver's has the longest name, it has ${hacker2.length} characters.`)
    }else{
...
/**
 * Tambien lo intentamos hacer mediante un loop, pero no supimos cómo aterrizarlo
 * 
let nameArray = {
for (i = nameUpperCase.length - 1; i >=0; i-- ){
  console.log(nameUpperCase.charAt(i))
}
for (i = nameUpperCase2.length - 1; i >=0; i-- ){
  console.log(nameUpperCase2.charAt(i))
}
}
*/
...
function isPalindrome (value) {
    const cleanWords = value.toUpperCase().replace(/ /g, '')
    console.log(cleanWords)
    const reversed = cleanWords.split('').reverse().join(''); 
    console.log(reversed);
    return reversed === cleanWords;
    }
